...
const JourneyStep: React.FC<JourneyStepProps> = ({ icon, title, description, isActive, index }) => {
  return (
    <div className={`flex flex-col items-center transition-all duration-300 ${isActive ? 'opacity-100 scale-105' : 'opacity-60 scale-100'}`}>
      <div className={`relative w-16 h-16 md:w-20 md:h-20 flex items-center justify-center rounded-full bg-gray-50 shadow-md transition-all duration-300 ${isActive ? 'bg-green-50 text-whatsapp border-whatsapp shadow-lg' : 'text-gray-500 border-gray-200'}`}>
        <div className="text-2xl md:text-3xl">{icon}</div>
        {index < 4 && (
          <div className={`absolute left-full top-1/2 w-12 md:w-16 lg:w-24 h-0.5 transform -translate-y-1/2 transition-all duration-300 ${isActive ? 'bg-whatsapp' : 'bg-gray-300'} hidden md:block`}>
            <div className="absolute right-0 top-1/2 transform -translate-y-1/2 -translate-x-1/2 w-2 h-2 rounded-full bg-whatsapp"></div>
          </div>
        )}
      </div>
      <h3 className={`mt-4 font-medium text-center transition-all duration-300 ${isActive ? 'text-whatsapp' : ''}`}>{title}</h3>
        ...
        return (
          <div className="bg-white rounded-lg p-6 shadow-lg animate-fade-in">
            ...
            <div className="mt-4 flex justify-center">
              <button className="px-4 py-2 bg-whatsapp text-white rounded-lg shadow">Create Your WhatsApp Flow</button>
            </div>
          </div>
        );
          ...
          <div className="bg-white rounded-lg p-6 shadow-lg animate-fade-in">
            ...
            <div className="mt-4 bg-gray-100 p-3 rounded-lg">
              ...
              <div className="flex items-start justify-end">
                <div className="bg-whatsapp text-white p-2 rounded-lg">A customer support flow for my e-commerce store</div>
              </div>
            </div>
          </div>
        );
        ...
        return (
          <div className="bg-white rounded-lg p-6 shadow-lg animate-fade-in">
            ...
            <div className="mt-4 flex flex-wrap justify-center gap-3">
              <button className="px-4 py-2 bg-whatsapp text-white rounded-lg shadow">Deploy Flow</button>
              <button className="px-4 py-2 bg-white border border-whatsapp text-whatsapp rounded-lg shadow">A/B Test</button>
            </div>
          </div>
        );
            ...
            <div className="mt-4 bg-gray-100 p-3 rounded-lg h-auto sm:h-32 flex items-center justify-center">
              <div className="grid grid-cols-1 sm:grid-cols-3 gap-3 w-full">
                <div className="bg-white p-2 rounded text-center">
                  <div className="text-whatsapp font-bold">87%</div>
                  <div className="text-xs text-gray-600">Response Rate</div>
                </div>
                <div className="bg-white p-2 rounded text-center">
                  <div className="text-whatsapp font-bold">2.5m</div>
                  <div className="text-xs text-gray-600">Avg. Response Time</div>
                </div>
                <div className="bg-white p-2 rounded text-center">
                  <div className="text-whatsapp font-bold">68%</div>
          ...
          <div className="flex justify-center mt-6">
            <div className="flex space-x-2">
              {journeySteps.map((_, index) => (
                <div 
                  key={index}
                  className={`w-2 h-2 rounded-full ${activeStep === index ? 'bg-whatsapp' : 'bg-gray-300'}`}
                />
              ))}
            </div>
          </div>
